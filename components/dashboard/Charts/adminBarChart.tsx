...
const AdminBarChart = ({ userData }) => {
  ...
  const spentCoins = totalSales?.adminSellerDate.map(
    (e: { coins: number; total: number }) => e.coins,
  )
  const totalSalesPDay = totalSales?.adminSellerDate.map(
    (e: { total: number; coins: number }) => e.total,
  )
