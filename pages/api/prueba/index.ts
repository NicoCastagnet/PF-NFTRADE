...
export default async function payDescription(
  ...
  if (req.method === 'POST') {
    ...
    const topic = query
    try {
      if (topic.type === 'payment' && body.user_id !== undefined) {
        ...
        if (payment.data.additional_info.items[0].quantity) {
          const data = await prisma.buys.create({
            data: {
              buyId: topic['data.id'] as string,
          ...
          if (data.status === 'approved') {
            if (!userCoins) {
              ...
              const totalCoins: number = data.coins + userCoins?.coins
